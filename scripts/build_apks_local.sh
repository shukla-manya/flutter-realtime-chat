...
build_one() {
  ...
  # Use local gradle zip (sandbox-friendly / offline)
  sed -i '' 's|distributionUrl=.*|distributionUrl=file\\:///Users/manyashukla/dev-tools/gradle-8.12-all.zip|' android/gradle/wrapper/gradle-wrapper.properties
  # Ensure gradle install marker
  DIST="/Users/manyashukla/.gradle/wrapper/dists/gradle-8.12-all/bm591p7ru188z8nkqq84epxyh"
  mkdir -p "$DIST"
  if [[ ! -d "$DIST/gradle-8.12" ]]; then
    unzip -q -o /Users/manyashukla/dev-tools/gradle-8.12-all.zip -d "$DIST"
  fi
  chmod +x "$DIST/gradle-8.12/bin/gradle" || true
  touch "$DIST/gradle-8.12-all.zip.ok"
  if [[ ! -f "$DIST/gradle-8.12-all.zip" ]]; then
    cp /Users/manyashukla/dev-tools/gradle-8.12-all.zip "$DIST/"
  fi
...
# Copy APKs to easy-to-find folder
mkdir -p "$ROOT/apks"
